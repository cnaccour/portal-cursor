...
function displayInvitations(invitations) {
  ...
  const html = invitations.map(inv => {
    ...
    if (inv.status === 'pending' && !isExpired) {
      actions.push(`<button onclick=\"copyInvitationLink('${inv.token}')\" class=\"px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200\">Copy Link</button>`);
      actions.push(`<button onclick=\"revokeInvitation(${inv.id}, '${escapeHtml(inv.email)}')\" class=\"px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700\">Revoke</button>`);
    } else {
      actions.push(`<button onclick=\"reinviteInvitation(${inv.id})\" class=\"px-4 py-2 text-sm bg-black text-white rounded-lg hover:bg-gray-800\">Reinvite</button>`);
      actions.push(`<button onclick=\"deleteInvitation(${inv.id})\" class=\"px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200\">Delete</button>`);
    }
    return `<div class=\"p-4 sm:p-6 hover:bg-gray-50 transition-colors\"><div class=\"flex flex-col sm:flex-row sm:items-center gap-4\"><div class=\"flex-grow\"><div class=\"flex flex-wrap items-center gap-2\"><h3 class=\"font-semibold text-gray-900\">${escapeHtml(inv.email)}</h3><span class=\"px-2 py-1 text-xs font-medium rounded-full ${badge}\">${inv.status.charAt(0).toUpperCase()+inv.status.slice(1)}</span></div><div class=\"mt-1 text-sm text-gray-500\">Expires ${formatDate(inv.expires_at)}</div></div><div class=\"flex items-center gap-3\">${actions.join('')}<\/div><\/div><\/div>`;
  }).join('');
...
function reinviteInvitation(id){const fd=new FormData();fd.append('invitation_id',id);fd.append('csrf_token','<?= $_SESSION['csrf_token'] ?>');fetch('api/invitations/reinvite-invitation.php',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success){showBanner('success','Invitation re-sent');loadInvitations();}else{showBanner('error',d.message||'Failed to reinvite');}}).catch(()=>showBanner('error','Error reinviting invitation'))}

function deleteInvitation(id){if(!confirm('Delete this invitation permanently?'))return;const fd=new FormData();fd.append('invitation_id',id);fd.append('csrf_token','<?= $_SESSION['csrf_token'] ?>');fetch('api/invitations/delete-invitation.php',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success){showBanner('success','Invitation deleted');loadInvitations();}else{showBanner('error',d.message||'Failed to delete');}}).catch(()=>showBanner('error','Error deleting invitation'))}
