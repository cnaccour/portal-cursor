...
git clone <repo-url> portal || true
cd ~/repos/portal && git checkout cpanel-test-deploy && git pull
# map web root to public/
ln -snf ~/repos/portal/public ~/public_html
```
...
# Pull latest changes on current branch (cpanel-test-deploy)
git pull
...
### ✅ Automated migrations (preferred)
```bash
# Option A: schema-only (applies db/schema.sql)
...
# Option B: full numbered migrations (applies database/migrations/*.sql)
...
Note: In production, the app prefers `public/includes/db.php` with cPanel credentials and DSN fallbacks. Only edit `includes/db.php` for local/dev.
```php
...
### Regular Updates
```bash
# To update the portal with latest code on cpanel-test-deploy:
cd ~/repos/portal
git pull
